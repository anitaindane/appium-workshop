class ScreenSetFilterParameters < ScreenBase
  ...
  def set_value(text_view_name, from, to)
    @driver.find_elements(@parameter_holders[:type], @parameter_holders[:value]
    ).each do |row|
      next unless row.find_element(:id, 'parameter_name').text == text_view_name
